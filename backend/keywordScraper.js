...
async function scrapeKeyword(keyword, igUsername, maxPosts = 10) {
  ...
  try {
    ...
    if (!searchInput) {
      ...
      const searchUrl = `https://www.instagram.com/explore/search/keyword/?q=${encodedKeyword}`;
    ...
    while (scrollAttempts < maxScrollAttempts && postUrls.size < maxPosts) {
      ...
      console.log(
        `Search scroll ${scrollAttempts + 1}: Found ${urlsOnPage.length} post links. Total unique URLs: ${postUrls.size}/${maxPosts}`
      );
    ...
    console.log(
      `✅ Keywords search completed. Found ${finalUsernames.length} unique usernames`
    );
