...
const { AutomationService } = require("./database/automation");
...
class MessageMonitor {
  ...
  async processIncomingMessage(message, fromUsername, monitorUsername, page) {
    try {
      ...
      console.log(`Found ${triggeredRules.length} triggered rules`);

      for (const rule of triggeredRules) {
        await this.executeAutomationRule(rule, fromUsername, message, page);
      }
  ...
  async executeAutomationRule(rule, fromUsername, message, page) {
    try {
      console.log(`Executing automation rule: ${rule.name}`);

      const actionParams = JSON.parse(rule.action_parameters || '{}');
      
      if (rule.action_type === 'send_message' && actionParams.response_template) {
        // Send automated response
        const responseText = this.personalizeMessage(actionParams.response_template, fromUsername);
        
        await this.sendResponse(page, responseText);
        
        // Record the execution
        await AutomationService.recordExecution(rule.id, fromUsername, {
          trigger_message: message.text,
          response_sent: responseText,
          status: 'success'
        });

        console.log(`Sent automated response to ${fromUsername}: "${responseText}"`);
      }
