...
async function scrapeHashtag(hashtag, igUsername, maxPosts = 10) {
  ...
  console.log(`Account found:`, account ? `Yes (has cookies: ${!!account.cookies})` : 'No');
  
  if (!account) {
    throw new Error(`Account ${igUsername} not found in database. Please add the account first.`);
  }
  ...
  console.log(`Found ${account.cookies.length} cookies for account ${igUsername}`);
  ...
  try {
    ...
    if (pageTitle.includes('Page Not Found') || pageTitle.includes('Instagram')) {
      // Page might be restricted or not found, try to continue anyway
      console.log('Hashtag page may be restricted, attempting to scrape anyway...');
    }
    ...
    while (scrollAttempts < maxScrollAttempts && postUrls.size < maxPosts) {
      ...
      console.log(`Scroll ${scrollAttempts + 1}: Found ${urlsOnPage.length} post links. Total unique URLs: ${postUrls.size}/${maxPosts}`);
    ...
    console.log(`✅ Hashtag scraping completed. Found ${finalUsernames.length} unique usernames`);
