...
async function scrapeFollowers(profileUrl, igUsername) {
  ...
  try {
      ...
      try {
        followersButton = await page.evaluateHandle(() => {
          ...
          for (const element of elements) {
            ...
            if (text.includes('followers') && (element.tagName === 'A' || element.tagName === 'BUTTON' || element.tagName === 'SPAN')) {
              return element;
            }
          }
          return null;
    ...
    while (scrollAttempts < maxScrollAttempts) {
      ...
      await page.evaluate(() => {
        const modal = document.querySelector('div[role="dialog"] div > div > div:nth-child(2)');
        if (modal) {
